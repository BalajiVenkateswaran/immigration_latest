...
import { PetitionDetails2ColumnService } from "./petition-details.service";
...
export class PetitionDetails2ColumnComponent implements OnInit {
    isNotesEdit;
    private users;
    ...
    constructor(public appService: AppService, private petitionDetailsService2column: PetitionDetails2ColumnService,
    ...
    ngOnInit() {
        ...
        this.petitionDetailsService2column.getPetitionDetails(this.appService.petitionId)
            .subscribe((res) => {
                ...
                if (res['petitionTypeId'] != undefined) {
                    this.petitionDetailsService2column.getPetitionStages(this.headerService.user.accountId, res['petitionTypeId'])
                        .subscribe((res) => {
                            this.petitionStages = res['petitionStageList'];
                        });
                }
                this.petitionDetailsService2column
        ...
        this.petitionDetailsService2column.getAllPetitionTypesAndSubTypes()
            .subscribe((res) => {
    ...
    getPetionDelOrgs() {
        this.petitionDetailsService2column.getDelegatedOrgs(this.headerService.user.accountId, this.appService.petitionId).subscribe((res) => {
    ...
    sendchecklist() {
        this.petitionDetailsService2column.sendChecklist(this.headerService.user.accountId, this.appService.petitionId).subscribe((res) => {
    ...
    onSavePetitionInfoClick() {
        ...
        if (this.petitionDetails['name'] == '' || this.petitionDetails['name'] == null || this.petitionDetails['name'] == undefined && this.petitionDetails['status'] == '' || this.petitionDetails['status'] == null || this.petitionDetails['status'] == undefined && this.petitionDetails['status'] == '' || this.petitionDetails['status'] == null || this.petitionDetails['status'] == undefined) {
            ...
            this.sfmpi = false;
            this.petitionDetailsService2column.savePetitionDetails(this.petitionDetails, this.headerService.user.userId)
                .subscribe((res) => {
                    if (res['petitionInfo'] != undefined) {
                        this.isPetitionInformationSave = false;
                        this.petitionDetails = res['petitionInfo'];
                        this.petitionDetailsService2column.getUsersForAccount(this.headerService.user.accountId)
                            .subscribe((res) => {
                                this.users = res['users'];
                                this.users.filter(user => {
                                    if (user.emailId == this.petitionDetails['assignedTo'])
    ...
    onSaveReceiptInfoClick() {
        ...
        if (this.receiptInfo.showReceiptNumberToClient != 0 && this.receiptInfo.showReceiptNumberToClient != 1) {
            ...
            this.sfmRI = false;
            this.petitionDetailsService2column.saveReceiptInfo(this.receiptInfo, this.headerService.user.userId)
                .subscribe((res) => {
                    if (res['receiptInfo'] != undefined) {
                        this.isReceiptInfoSave = false;
                        this.receiptInfo = res['receiptInfo'];
    ...
    onSaveLCAInfoClick() {
        ...
        this.petitionDetailsService2column.saveLcaInfo(this.lcaInfo, this.headerService.user.userId)
            .subscribe((res) => {
                this.isLCAInfoEdit = true;
                if (res['lcaInfo'] != undefined) {
                    this.lcaInfo = res['lcaInfo'];
    ...
    onSaveSponsorInfoClick() {
        ...
        this.petitionDetailsService2column.saveSponsorInfo(this.sponsorInfo, this.headerService.user.userId)
            .subscribe((res) => {
                this.isSponsorInfoEdit = true;
                if (res['sponsorInfo'] != undefined) {
                    this.sponsorInfo = res['sponsorInfo'];
    ...
    onSaveAdditionalDetailsClick() {
        ...
        if (this.petitionAdditionalDetails['showReceiptNumberToClient'] == '' || null || undefined) {
            ...
            this.sfmRI = false;
            this.petitionDetailsService2column.savePetitionAdditionalDetails(this.petitionAdditionalDetails, this.headerService.user.userId)
                .subscribe((res) => {
                    this.isAdditionalDetailsEdit = true;
                    if (res['petitionAdditionalDetails'] != undefined) {
                        this.petitionAdditionalDetails = res['petitionAdditionalDetails'];
    ...
    onSaveDelegatedOrgsClick() {
        ...
        this.petitionDetailsService2column.saveDelegatedOrgs(this.orgs, this.appService.petitionId, this.headerService.user.userId)
            .subscribe((res) => {
